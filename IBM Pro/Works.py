...
#df=pd.read_csv("C://Users/Admin/Downloads/Amazon Sales Dataset.csv")
np.random.seed(0)
num_records = 50

data = {
    'Order_ID': np.random.randint(1000, 2000, num_records),
    'Product_Category': np.random.choice(['Electronics', 'Books', 'Clothing',
                                          'Beauty', 'Home',
                                          'Toys', 'Sports', 'Food', 'Health',
                                          'Jewelry',
                                          'Automotive', 'Furniture', 'Tools',
                                          'Garden',
...
# Stacked Bar Chart
stacked_data = df.groupby(['Product_Category']).agg({'Quantity':'sum'}).
reset_index()

fig = px.bar(stacked_data, x='Product_Category', y='Quantity',
             title='Total Quantity Sold by Product Category',
             labels={'Product_Category': 'Product Category', 'Quantity': 'Total
             Quantity Sold'},
...
# Sankey Diagram
sankey_data = df.groupby(['Product_Category', 'Order_ID']).size().reset_index
 (name='Count')
fig = px.parallel_categories(sankey_data, dimensions=['Product_Category',
                                                      'Order_ID'],
                             color='Count',
                             labels={'Product_Category':'Product Category',
                                     'Order_ID':'Order ID', 'Count':'Count'})
fig.show()

# Create hover text for heatmap
hover_text = [[f'Product: {col}<br>Order ID: {row}<br>Quantity:
{heatmap_data.loc[col, row]}
          ' for row in heatmap_data.columns] for col in heatmap_data.index]
...
fig = px.pie(names=pie_data.index, values=pie_data.values,
             title='Sales Distribution by Product Category',
             labels={'names':'Product Category', 'values':'Sales'})
fig.show()

# Bubble Chart
bubble_data = df.groupby(['Product_Category']).agg({'Quantity':'sum',
                                                    'Price':'mean'}).
                                                    reset_index()
fig = px.scatter(bubble_data, x='Quantity', y='Price', size='Quantity',
                 color='Product_Category',
                 hover_name='Product_Category',
                 title='Bubble Chart of Sales Data',
                 labels={'Quantity':'Total Quantity Sold', 'Price':'Average
                 Price', 'Product_Category':'Product Category'})
fig.show()
...
# Violin Plot with hover
fig = px.violin(df, x='Product_Category', y='Price', title='Violin
Plot of Price by Product Category',
                hover_name='Product_Category', hover_data={'Price': True},
                labels={'Product_Category': 'Product Category',
                        'Price': 'Price'})
...
# Box Plot with hover
fig = px.box(df, x='Product_Category', y='Price', title='Box Plot of Price by Product Category',
             labels={'Product_Category': 'Product Category', 'Price': 'Price'},
             hover_data={'Price': True})
fig.show()

# Bar Chart with hover
bar_data = df.groupby('Product_Category')['Price'].sum().reset_index()
fig = px.bar(bar_data, x='Product_Category', y='Price', title='Total
Sales by Product Category',
             labels={'Product_Category': 'Product Category', 'Price':
                     'Total Sales'},
             hover_data={'Price': True})
fig.show()
...
fig = px.scatter(df, x='Quantity', y='Price', color='Product_Category',
                 ...
                 labels={'Quantity': 'Quantity', 'Price': 'Price', 'Product_Category': 'Product Category'},
                 hover_name='Product_Category', hover_data={'Quantity': True, 'Price': True})
fig.show()
